...
(async()=>{
 ...
 const report={pages:[],homeSizes:[],interactions:[],errors:[],failedLocal:[]};
 try {
  ...
  for(const[width,height]of [[1440,900],[1366,768],[1280,720],[1024,768]]){
   await page.setViewportSize({width,height});for(const locale of ['en','id']){
    ...
    const metrics=await page.evaluate(()=>{const s=document.querySelector('#services'),r=s.getBoundingClientRect();return{height:r.height,viewport:innerHeight,width:document.documentElement.scrollWidth,cards:[...s.querySelectorAll('.service-panel')].map(e=>({client:e.clientHeight,scroll:e.scrollHeight}))};});
    report.homeSizes.push({width,height,locale,...metrics});assert(metrics.height<=height-72+1,JSON.stringify(metrics));assert.equal(metrics.width,width);assert(metrics.cards.every(c=>c.scroll<=c.client+1),'clipped card content');
   }
  }
  ...
  for(const locale of ['en','id']){
   for(const[name,url,sel]of selections){
    ...
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth),390,locale+name+' mobile overflow');
   }
  }
