...
const actualPages = values.slice(1).map((row) => normalize(row[1]));
const actualNewCopy = values.slice(1).map((row) => normalize(row[6]));
const expectedPages = expected.copyMaster.slice(1).map((row) => normalize(row[1]));
const expectedNewCopy = expected.copyMaster.slice(1).map((row) => normalize(row[6]));
...
const badCopyRows = actualNewCopy.map((copy, index) => ({ row: index + 2, copy })).filter((item) => forbidden.test(item.copy));
const errorCells = values.flatMap((row, rowIndex) => row.map((value, colIndex) => ({ row: rowIndex + 1, col: colIndex + 1, value }))).filter((cell) => formulaErrors.has(cell.value));
const blanks = actualNewCopy.map((copy, index) => ({ row: index + 2, copy })).filter((item) => item.copy.trim().length === 0).map((item) => item.row);
const checks = {
...
const passed = checks.sheets.join('|') === 'Copy Master|Page Index|Notes' && checks.rowCount === 597 && checks.pagesMatch && checks.newCopyMatches && checks.pageIndexMatches && checks.notesMatch && checks.lockedHeroEn && checks.lockedHeroId && checks.badCopyRows.length === 0 && checks.errorCells.length === 0 && JSON.stringify(checks.blanks) === JSON.stringify([300, 308, 454, 462]);
