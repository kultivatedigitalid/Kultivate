...
// 01 — Series orientation and lesson focus.
{
 ...
 text(s,'Gimana pencarian bisa\nbuka peluang bisnis',64,211,1150,185,61,C.white,true,'left','slide-title');
 text(s,'Satu lesson, satu keputusan atau cara memahami masalah',66,412,1080,52,29,C.gray);
...
{
 const s=base(2,'Traffic naik, peluangnya belum jelas','Contoh situasi');
 text(s,'YANG UDAH KELIHATAN',65,300,504,35,20,C.gray,true);
 text(s,'Website kita makin\nbanyak dikunjungi',65,356,512,150,44,C.white,true);
 s.shapes.add({geometry:'line',position:{left:620,top:290,width:0,height:272},line:{fill:C.line,width:2}});
 text(s,'YANG MASIH PERLU DICEK',683,300,514,35,20,C.cyan,true);
 text(s,'Siapa yang datang,\ndan mereka butuh apa?',683,356,514,145,37,C.white,true);
 text(s,'Inquiry yang masuk cocok nggak?',683,520,514,85,28,C.gray);
 note(s,2);
}
// 03 — Distinguish demand from a visit count.
{
 ...
 text(s,'Kebutuhan orang akan solusi',65,386,500,82,32);
 ...
 text(s,'Kebutuhannya cocok sama layanan kita nggak?',295,554,900,76,33,C.white);
 note(s,3);
}
// 04 — Editable framework with a return path.
{
 ...
 const small=['Kenali\nmasalah','Pahami\nsolusinya','Cek kecocokan\ndan buktinya','Ambil langkah\nberikutnya'];
 const nodes=[];
 for(let i=0;i<4;i++){
 ...
 text(s,'Orang bisa bolak-balik atau lewati beberapa momen',64,601,1140,37,25,C.gray);
 note(s,4,['S4']);
}
// 05 — One comparison table with native editable cells.
{
 const s=base(5,'Jawaban yang calon pembeli butuhkan','Kultivate framework');
 const vals=[['Discover','Masalah apa yang perlu diberesin?'],['Learn','Solusinya kerja gimana?'],['Compare/Validate','Cocok dan bisa dipercaya nggak?'],['Act','Gimana cara mulainya?']];
 ...
 text(s,'Satu halaman bisa bantu lebih dari satu momen',64,603,1152,39,26,C.gray);
 note(s,5);
}
// 06 — An evidence flow rather than a quantitative funnel.
{
 const s=base(6,'Dari muncul di search\nsampai peluang yang cocok','Kultivate framework  •  Tiap bagian perlu dicek • Bukan funnel terukur');
 const xs=[64,357,650,943];
 const labels=['Visibility','Kunjungan\nrelevan','Inquiry','Qualified\nopportunity'];
 const captions=['Muncul di search','Kebutuhannya nyambung','Pesan beneran masuk','Kecocokannya dicek'];
 const nd=[];
 for(let i=0;i<4;i++){
 ...
 text(s,'Tiap langkah perlu bukti',64,581,650,37,28,C.cyan,true);
 text(s,'Peluang yang cocok belum tentu jadi penjualan',64,620,1100,34,24,C.gray);
 note(s,6,['S2']);
}
// 07 — Events are separated from business validation.
{
 const s=base(7,'Klik kontak belum tentu jadi inquiry','Contoh ilustratif');
 const xs=[65,460,855],labs=['Klik tombol\nkontak','Pesan beneran\nmasuk','Tim cek\nkebutuhannya'];
 const nd=[];
 for(let i=0;i<3;i++){
 ...
 text(s,'Key event cuma nyatet tindakan penting,\nbelum otomatis jadi peluang yang cocok',65,566,1150,79,31,C.gray);
 note(s,7,['S3']);
}
// 08 — First half of the worked example.
{
 ...
 text(s,'Perlu paham masalahnya',65,527,510,70,29,C.gray);
 text(s,'02  LEARN',690,300,500,35,23,C.cyan,true);
 text(s,'Perawatan berkala\ntermasuk apa aja',690,362,510,132,43,C.white,true);
 text(s,'Perlu tahu apa aja yang dikerjain',690,527,510,83,29,C.gray);
 note(s,8);
}
// 09 — Same example moves to validation and action.
{
 const s=base(9,'Inquiry masuk, cocoknya tetap perlu dicek','Contoh hipotetis');
 text(s,'03  COMPARE / VALIDATE',65,304,575,34,22,C.cyan,true);
 text(s,'Area yang dilayani\nPekerjaan yang termasuk\nBukti yang bisa dicek',65,369,590,175,34,C.white,true);
 text(s,'04  ACT',734,304,470,34,22,C.cyan,true);
 text(s,'Kirim kebutuhan\nTahu proses setelahnya',734,369,470,155,33,C.white,true);
 line(s,65,559,1136,C.line,2);
 text(s,'Permintaan masuk tetap perlu dicek cocoknya',65,591,1136,67,31,C.gray);
 note(s,9);
}
// 10 — Observation, hypothesis and a useful next check.
{
 const s=base(10,'Yang kita lihat dan yang masih kita duga','Contoh hipotetis');
 const rows=[['OBSERVASI','Info area layanan belum ada di halaman'],['HIPOTESIS','Calon pembeli ragu lalu batal hubungi bisnis'],['LANGKAH CEK','Cek pertanyaan soal lokasi di inquiry yang masuk']];
...
{
 const s=base(11,'Sekarang coba di layanan Anda','Search-to-Sales Mapping Worksheet  •  Bagian C01L01');
 const steps=['Pilih satu layanan','Jawab empat pertanyaan pembeli','Catat yang masih kurang jelas','Pilih bukti yang mau kita cek'];
 ...
 text(s,'Belum tahu cara\npelanggan pakai search?',852,306,353,100,27,C.white,true);
 text(s,'Cari tahu dulu.',852,404,353,43,27,C.cyan);
 text(s,'Inquiry yang cocok\nlama dibalas?',852,479,353,85,28,C.white,true);
 text(s,'Cek tindak lanjutnya.',852,582,353,50,25,C.cyan);
 note(s,11);
}
// 12 — One takeaway and a learning CTA.
{
 const s=base(12,'');
 text(s,'Search bisa bantu orang\nambil keputusan',64,134,1152,160,57,C.white,true,'left','slide-title');
 text(s,'Cek apakah inquiry-nya cocok sama bisnis kita.',64,338,1120,60,35,C.cyan);
 text(s,'Traffic aja belum cukup buat bilang ada peluang bisnis.',64,410,1136,62,32,C.gray);
 line(s,64,519,1136);
 ...
 text(s,'Worksheet boleh dicoba, belajar tetap bisa lanjut',64,644,1000,24,17,C.gray);
 note(s,12);
}
